...
def main():
    ...
    x_dummy = tf.Variable(
        tf.zeros(shape=[batch_size, 1, 255, 1], dtype=tf.float32))
    y_dummy = tf.Variable(
        tf.zeros(shape=[batch_size, 256, 1, 1], dtype=tf.float32))
    ...
    with tf.Session() as sess:
        ...
        for it in range(iteration):
            ...
            tp_x_batch = tf.concat(values=[tp_x_batch, x_dummy], axis=1)
            tp_x_batch = tf.concat(values=[tp_x_batch, y_dummy], axis=2)
            tp_x_batch = tp_x_batch.eval()
            ...
            _, D_loss_current = sess.run([SIGN_categorical_model.D_solver, SIGN_categorical_model.D_loss], feed_dict={
                                         SIGN_categorical_model.X: tp_x_batch, SIGN_categorical_model.Y: Y_label, SIGN_categorical_model.Z: Z_sample})
            ...
            tn_x_batch = tf.concat(values=[tn_x_batch, x_dummy], axis=1)
            tn_x_batch = tf.concat(values=[tn_x_batch, y_dummy], axis=2)
            tn_x_batch = tn_x_batch.eval()
            ...
            _, D_loss_current = sess.run([SIGN_categorical_model.D_solver, SIGN_categorical_model.D_loss], feed_dict={
                                         SIGN_categorical_model.X: tn_x_batch, SIGN_categorical_model.Y: Y_label, SIGN_categorical_model.Z: Z_sample})
